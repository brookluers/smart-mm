...
allfiles <- list.files(pattern='sim1-.+\\.RData')
...
for (ncur in ssizes) {
  nfnames <-
    list.files(pattern = paste("N", ncur,'-', sep = '')) # all files with this sample size
  
  allres <- vector('list', length(nfnames)) ## a list of the results for each effect size, fixed sample size
  for (i in seq_along(allres)) {
    load(nfnames[i])
    cur_effsize <- str_extract(nfnames[i],'small|med|large')
    allres[[i]] <- list(truecoefs = get(paste("truecoefs_",cur_effsize,sep='')),
  ...
  allres_fmt <-
    bind_rows(lapply(allres, function(x) { # for each of the effect sizes at a given sample size
      true_eos_difference <- Lmat1 %*% x$truecoefs - Lmat2 %*% x$truecoefs
      resfmt <-
        bind_rows(lapply(x$res, function(sim_i) { ## For each of the simulated data sets
          ...
          contrast_results <-
            data.frame(coef = 'contrast_estimate',
                       vdiffnorm = sim_i$vnorms,
                       est = as.numeric(Lmat1 %*% t(sim_i$bhat) - Lmat2 %*% t(sim_i$bhat)),
                       N = ncur, method = sim_i$method)
          fmt_i <- bind_rows(fmt_i,contrast_results)
          
          ##
          # Collate the standard errors for each coefficient and the contrast
          sedat_contrast <- 
            data.frame(sehat = sqrt(sapply(sim_i$vlist, function(vm) return((Lmat1 - Lmat2) %*% vm %*% t(Lmat1 - Lmat2)))),
                       method = sim_i$method,
                       coef = 'contrast_estimate',
                       trueval = true_eos_difference)
          sedat_beta <-
            data.frame(
              sehat = as.numeric(sapply(sim_i$vlist, function(vm) return(setNames(sqrt(diag(vm)), names(x$truecoefs))))),
          ...
          fmt_i <-
            fmt_i %>% left_join(sedat,
                                by=c('method','coef')) %>%
            mutate(cilwr = est - zmult * sehat,
                   ciupr = est + zmult * sehat,
                   cover = 1 * ((cilwr <= trueval) & (trueval <= ciupr)))
          return(fmt_i)
        }))
      return(resfmt)
    }),
...
save(res_byN, allfiles, file='res-fmt.RData')
